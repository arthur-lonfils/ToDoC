...
{
    switch (p) {
    case PRIORITY_CRITICAL: return CLR_BOLD_RED;
    case PRIORITY_HIGH:     return CLR_RED;
    case PRIORITY_MEDIUM:   return CLR_YELLOW;
    case PRIORITY_LOW:      return CLR_DIM;
    default:                return "";
    }
}
...
{
    switch (s) {
    case STATUS_TODO:        return CLR_WHITE;
    case STATUS_IN_PROGRESS: return CLR_CYAN;
    case STATUS_DONE:        return CLR_GREEN;
    case STATUS_BLOCKED:     return CLR_RED;
    case STATUS_CANCELLED:   return CLR_DIM;
    default:                 return "";
    }
}
...
{
    switch (t) {
    case TASK_TYPE_BUG:     return CLR_RED;
    case TASK_TYPE_FEATURE: return CLR_BLUE;
    case TASK_TYPE_CHORE:   return CLR_WHITE;
    case TASK_TYPE_IDEA:    return CLR_MAGENTA;
    default:                return "";
    }
}
...
void display_task_row(const task_t *task)
{
    ...
    /* Status */
    printf("%s%-12s%s ", clr(status_color(task->status)),
           status_to_str(task->status), rst);

    /* Priority */
    printf("%s%-9s%s ", clr(priority_color(task->priority)),
           priority_to_str(task->priority), rst);

    /* Type */
    printf("%s%-8s%s ", clr(type_color(task->type)),
           task_type_to_str(task->type), rst);
...
void display_task_detail(const task_t *task)
{
    ...
    printf("  %-12s %s%s%s\n", "Status:",
           clr(status_color(task->status)), status_to_str(task->status), rst);
    printf("  %-12s %s%s%s\n", "Priority:",
           clr(priority_color(task->priority)), priority_to_str(task->priority), rst);
    printf("  %-12s %s%s%s\n", "Type:",
           clr(type_color(task->type)), task_type_to_str(task->type), rst);
...
void display_task_list(const task_t *tasks, int count)
{
    ...
    printf("\n  %s%s%-6s %-12s %-9s %-8s %s%s\n",
           clr(CLR_BOLD), clr(CLR_DIM),
           "ID", "STATUS", "PRIORITY", "TYPE", "TITLE", rst);
    printf("  %s%s%s\n", clr(CLR_DIM),
           "─────────────────────────────────────────────────────────────", rst);
...
void display_stats(const task_stats_t *stats)
{
    ...
    for (int i = 0; i < STATUS_COUNT; i++) {
        if (stats->by_status[i] > 0) {
            printf("    %s%-12s%s %d\n",
                   clr(status_color((status_t)i)),
                   status_to_str((status_t)i), rst,
                   stats->by_status[i]);
        }
    }
    ...
    for (int i = 0; i < PRIORITY_COUNT; i++) {
        if (stats->by_priority[i] > 0) {
            printf("    %s%-12s%s %d\n",
                   ...
                   priority_to_str((priority_t)i), rst,
                   stats->by_priority[i]);
        }
    }
    ...
    for (int i = 0; i < TASK_TYPE_COUNT; i++) {
        if (stats->by_type[i] > 0) {
            printf("    %s%-12s%s %d\n",
                   ...
                   task_type_to_str((task_type_t)i), rst,
                   stats->by_type[i]);
        }
    }
