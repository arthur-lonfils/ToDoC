...
/* ── Output mode (~/.todoc/mode) ───────────────────────────── */

char *todoc_mode_path(void)
{
    const char *home = getenv("HOME");
    if (!home) {
        fprintf(stderr, "todoc: HOME environment variable not set\n");
    ...
    size_t len = strlen(home) + strlen("/.todoc/mode") + 1;
    char *path = todoc_malloc(len);
    snprintf(path, len, "%s/.todoc/mode", home);
    return path;
}

output_mode_t todoc_get_mode(void)
{
    char *path = todoc_mode_path();
    if (!path) {
        return OUTPUT_MODE_USER;
    }
    FILE *fp = fopen(path, "r");
    free(path);
    if (!fp) {
    ...
    if (strcmp(buf, "ai") == 0) {
        return OUTPUT_MODE_AI;
    }
    return OUTPUT_MODE_USER;
}

int todoc_set_mode(output_mode_t mode)
{
    char *path = todoc_mode_path();
    if (!path) {
        return -1;
